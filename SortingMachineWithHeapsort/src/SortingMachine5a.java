...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        // Check to see that the tree is a length greater than 0
        if (last >= top) {
            // Get index of left subtree
            int indexLeft = top * 2 + 1;
            // Get index of right subtree
            int indexRight = indexLeft + 1;

            // If there is a right tree
            if (last >= indexRight) {
                // Check to see which node is smaller.
                // First check if the left tree is smaller

                if (order.compare(array[indexLeft], array[indexRight]) <= 0) {
                    // Make sure that root is greater than the left
                    if (order.compare(array[top], array[indexLeft]) > 0) {
                        // Switch places
                        exchangeEntries(array, top, indexLeft);
                        // Sift down
                        siftDown(array, indexLeft, last, order);
                    }
                    // If right tree is smaller swap with right tree
                } else {
                    // Make sure that root is greater than the right
                    if (order.compare(array[top], array[indexRight]) > 0) {
                        // Switch places
                        exchangeEntries(array, top, indexRight);
                        // Sift down
                        siftDown(array, indexRight, last, order);
                    }
                }
            } else {
                // This could will run if there is no right tree
                // Make sure that root is greater than the left
                if (indexLeft <= last) {
                    if (order.compare(array[top], array[indexLeft]) > 0) {
                        // Switch places
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        /*
         * Start at top of tree Check if left subtree is heap, if not heap
         * recursively called isHeap until it is a heap If not a heap fix
         * ordering. Do same for right subtree Then recursively call siftDown on
         * both trees
         */
        ...
        if (top <= last) {
            heapify(array, indexLeft, order);
            heapify(array, indexRight, order);
