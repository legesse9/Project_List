...
public class List3<T> extends ListSecondary<T> {
    ...
    private void createNewRep() {
        // Assign lengths for left and right list
        this.leftLength = 0;
        this.rightLength = 0;

        // Create nodes for preStart and postFinish
        this.preStart = new Node();
        this.postFinish = new Node();

        // Assign properties of preStart
        this.preStart.data = null;
        this.preStart.next = this.postFinish;
        this.preStart.previous = null;

        // Assign properties for postFinish
    ...
    public final void addRightFront(T x) {
        ...
        newRight.data = x;
        newRight.next = lastLeft.next;
        newRight.previous = this.lastLeft;

        // Update lastLeft
        lastLeft.next = newRight;

        // Update node after new one
    ...
    public final T removeRightFront() {
        ...
        // Grab removed data
        T data = lastLeft.next.data;

        // Grab node after the removed one
        Node nodeAfterRemoval = this.lastLeft.next.next;

        // Reassign last left values
        lastLeft.next = nodeAfterRemoval;

        // Reassign node after removed one
        nodeAfterRemoval.previous = lastLeft;
