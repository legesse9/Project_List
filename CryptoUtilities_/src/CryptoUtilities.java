...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber r = new NaturalNumber2(0);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        nCopy.decrement();
        NaturalNumber one = new NaturalNumber2(1);
        // calculating w^2 mod n
        w.multiply(wTwo);
        NaturalNumber r = w.divide(n);
        w.copyFrom(r);
        NaturalNumber wThree = new NaturalNumber2(wTwo);
        // calculating w^(n-1) mod n
        for (NaturalNumber i = new NaturalNumber2(1); i.compareTo(nCopy) < 0; i
                .increment()) {
            wTwo.multiply(wThree);
        }
        NaturalNumber x = wTwo.divide(n);
        wTwo.copyFrom(x);

        if (w.compareTo(one) == 0 || wTwo.compareTo(one) != 0) {
            isWitness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        nCopy.decrement();
        NaturalNumber one = new NaturalNumber2(1);
        //NaturalNumber rnd = randomNumber(fifty);
        NaturalNumber[] array = new NaturalNumber[ten];
        for (int i = 0; i < ten; i++) {
            NaturalNumber rnd = randomNumber(nCopy);
            while (rnd.compareTo(one) <= 0) {
                rnd = randomNumber(nCopy);
            }
            if (rnd.compareTo(one) > 0) {
                array[i] = rnd;
            }
        }
        // if
        ...
        for (int i = 0; i < ten; i++) {
            if (!isWitnessToCompositeness(array[i], n)) {
                isPrime = true;
            } else {
                isPrime = false;
            }
